...
def generate_2D_plot(key, 
    ...
    direction = generate_random_weights(model_state.params, key)
    ...
    for x in coords:
      new_params = perturb_params(init_weights, direction, x)
...
def generate_3D_plot(key,
    ...
    direction_x = generate_random_weights(model_state.params, key)
    sub, _ = jax.random.split(key)
    direction_y = generate_random_weights(model_state.params, sub)
    ...
    for i in range(X.shape[0]):
      for j in range(X.shape[1]):
        new_params = perturb_params(init_weights, [direction_x, direction_y], [X[i,j], Y[i,j]])
        loss = compute_loss(model_state, new_params, batch_stats, train_loader_unshuffled, num_batches)
